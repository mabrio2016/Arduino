...
void loop() {
  ...
  if(fase==1){
    if(y!=ny){ 
      tft.fillEllipse(nx, ny, 2, 2, TFT_BLACK);   //clearing the ball
      ny=y;
      nx=x;
    }
    if(int(px)!=pxn){
      tft.fillRect(pxn,234,24,4,TFT_BLACK);       //clearing players
      pxn=px;
    }

    if(px>=2 && px<=109){
      if(digitalRead(0)==0) px=px-1;
      if(digitalRead(35)==0) px=px+1;
    }
    if(px<=3) px=4;
      if(px>=108) px=107;  
    if(y>232 && x>px && x<px+24){                   //delete later
      ys=ys*-1;  
    ...
    for(int i=0;i<16;i++) {
      tft.fillRect(enx[i],eny[i],20,4,enc[i]);      //draw enemies
    }
